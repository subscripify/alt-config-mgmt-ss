...
import (
	...
	subscripifylogger "github.com/subscripify-private/subscripify-logger"
)
...
func init() {
	goSql := getNewMySQLTenantDbHandle()
	gormSql := getGormHandle(goSql)
	Tdb = tenantDb{goHandle: goSql, Handle: gormSql}
}

// initializes a gorm ORM connection using an existing connection
func getGormHandle(dbConnection *sql.DB) *gorm.DB {
	...
	if err != nil {
		subscripifylogger.FatalLog.Fatal("Failed to initialize ORM (GORM) for database connection")
	}
	return gormDB
}
